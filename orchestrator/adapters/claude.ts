...
/**
 * Adaptador Claude Code — vía `@anthropic-ai/claude-agent-sdk`.
 * No hay "crear sesión" explícito: el `session_id` nace en el primer `query()`
 * y se reutiliza con `resume` en los siguientes turnos. `createSession`
 * devuelve un handle interno que liga ese id de forma perezosa.
 */
export class ClaudeAdapter implements AgentAdapter {
  readonly kind = 'claude' as const;
  private state = new Map<string, ClaudeState>();
  private counter = 0;

  async createSession(opts: CreateSessionOpts): Promise<{ sessionId: string }> {
    const handle = `claude-${++this.counter}`;
  ...
  async *send(handle: string, text: string): AsyncIterable<Delta> {
    ...
    if (!st) {
      yield { type: 'error', message: 'Sesión Claude inexistente' };
      yield { type: 'done' };
      return;
    }
